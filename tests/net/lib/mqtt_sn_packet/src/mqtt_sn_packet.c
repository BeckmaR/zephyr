...
static void test_mqtt_packet_decode(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(decode_tests); i++) {
		...
		msg = mqtt_sn_msg_from_data(decode_tests[i].data, decode_tests[i].datasz);
		zassert_not_equal(msg, NULL, "Got no message");
...
static void test_mqtt_packet_encode(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(encode_tests); i++) {
		...
		msg = mqtt_sn_encode_msg(&encode_tests[i].p);
		zassert_not_equal(msg, NULL, "Got no message");
		mqtt_sn_msg_data(msg, &data);
		zassert_equal(data.size, encode_tests[i].expectedsz,
			      "Unexpected data size %zu (%zu)", data.size,
		...
		zassert_mem_equal(encode_tests[i].expected, data.data, data.size,
				  "Bad encoded message");
		mqtt_sn_msg_unref(msg);
	}
}
